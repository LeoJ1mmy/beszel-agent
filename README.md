...
**Ubuntu / Debian 一鍵安裝（x86_64）**

```bash
cd /tmp && curl -fsSL https://github.com/LeoJ1mmy/beszel-agent/releases/latest/download/beszel-agent_linux_x86_64.tar.gz | tar -xz && sudo sh install.sh
```

依提示輸入 Hub URL、Token、Public Key 即完成。

---

**其他 Linux 或 ARM 手動安裝**

1. 至 [Releases 頁面](https://github.com/LeoJ1mmy/beszel-agent/releases/latest) 下載對應 `.tar.gz`
2. 解壓縮：
   ```bash
   tar -xzf beszel-agent_linux_*.tar.gz
   ```
3. 執行安裝腳本：
   ```bash
   sudo sh install.sh
